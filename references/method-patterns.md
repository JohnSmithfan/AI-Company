...
To ensure PII masking works correctly, follow this verification method:

**Step 1: Identify PII Patterns**
- Email addresses: `user@example.com` → `[EMAIL]`
- IP addresses: `192.168.1.1` → `[IP]`
- Phone numbers: `13800138000` → `[PHONE]`
- ID numbers: `110101199001011234` → `[ID]`

**Step 2: Test Cases**
```python
test_cases = [
    ("Contact me at user@example.com", "Contact me at [EMAIL]"),
    ("Server IP is 192.168.1.1", "Server IP is [IP]"),
    ("My phone is 13800138000", "My phone is [PHONE]"),
]
```

**Step 3: Verification Method**
1. Run `mask_sensitive_data(text)` on each test case
2. Verify output matches expected result
3. Ensure original data is NOT logged anywhere
4. Check that masking is applied BEFORE any output or logging

**Step 4: Audit Trail**
- Log that masking was applied (without logging the original data)
- Include masking timestamp in audit trail
- Record masking version for reproducibility

> **Important**: PII masking must be applied at the point of data collection, before any processing or storage.
