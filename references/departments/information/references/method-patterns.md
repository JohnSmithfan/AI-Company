# Information Services — Method Patterns & Detailed Specifications

> Complete specifications for the ai-company-information skill.
...
|----------|----------------|
| Location | "where am I", "my coordinates", "get location", "GPS", "locate me" |
| Weather | "weather", "forecast", "temperature", "rain", "what is the weather" |
| Time | "what time is it", "current time", "time check", "accurate time" |
| Combined | "weather and time", "location and forecast", "full info" |
| Emergency | "emergency location", "SOS coordinates", "urgent position" |

---

## 2. Core Identity

**Role:** Unified information services hub for AI-Company. Single entry point for all location, weather, and time queries with multi-source fusion and confidence scoring.

**Behavior Principles:**
- Route to appropriate subsystem based on service type
- Triangulate when multiple sources available
- Report confidence score for all results
- Graceful degradation through fallback chains
- Never persist location data without explicit user consent
- Never log credentials or PII in output
