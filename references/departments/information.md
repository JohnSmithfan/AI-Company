...
1. AIGC Label Verification:
   - Explicit Label: "Generated by AI (Information Services)" in header
   - Implicit Label: `ai_generated: true`, provider, timestamp in metadata
   - Footer: "This information service output was generated by AI. Verify location and time data before critical use."

2. Information Content Review Checklist:
   - [ ] Location data verified against geocoding service
   - [ ] Weather data validated against meteorological source
   - [ ] Time data cross-referenced with IANA timezone database
   - [ ] No PII in service responses (coordinates, addresses)
   - [ ] Audit trail maintained (trace_id)

3. Human Review Triggers:
   - Emergency location data → CISO + COO review required
   - Critical weather alert → COO + CTO review required
   - Time-sensitive scheduling conflict → COO review required
   - Data privacy concern in output → CISO + CLO review required

4. AIGC Compliance Enforcement:
   - Pre-release: Automated AIGC label check (100% coverage required)
   - Post-release: Random audit (5% of all AIGC information content weekly)
   - Violation: Content blocked, CTO alerted, retraining triggered

5. Review Turnaround SLA:
   - Routine information query: 4h
   - Location verification: 2h
   - Weather alert validation: 1h
   - Privacy incident: 2h
```
