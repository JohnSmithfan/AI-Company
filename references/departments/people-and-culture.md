...
1. AIGC Label Verification:
   - Explicit Label: "Generated by AI (People & Culture)" in header
   - Implicit Label: `ai_generated: true`, provider, timestamp in metadata
   - Footer: "This people analytics was generated by AI. Verify sensitive findings before action."

2. People Content Review Checklist:
   - [ ] Personnel recommendations validated against performance data
   - [ ] Bias and fairness assessment completed
   - [ ] Privacy implications reviewed (no PII exposure)
   - [ ] Ethical alignment with company values
   - [ ] Audit trail maintained (trace_id)

3. Human Review Triggers:
   - Hiring/firing recommendation → CHO + CEO review required
   - Compensation analysis → CHO + CFO review required
   - Ethics violation report → CHO + CLO + CISO review required
   - Organizational health alert → CHO + CEO review required

4. AIGC Compliance Enforcement:
   - Pre-release: Automated AIGC label check (100% coverage required)
   - Post-release: Random audit (5% of all AIGC people content weekly)
   - Violation: Content blocked, CHO alerted, retraining triggered

5. Review Turnaround SLA:
   - Routine people report: 24h
   - Hiring recommendation: 48h
   - Ethics assessment: 12h
   - Crisis people analysis: 4h
```
