...
1. AIGC Label Verification:
   - Explicit Label: "Generated by AI (Translation & Localization)" in header
   - Implicit Label: `ai_generated: true`, provider, timestamp in metadata
   - Footer: "This translation was generated by AI. Verify accuracy before use in legal or official contexts."

2. Translation Content Review Checklist:
...
3. Human Review Triggers:
   - Legal/contract translation → CLO + human translator review required
   - Marketing brand translation → CMO + human translator review required
   - PII detected in source → CISO review required, sanitize and re-route
   - Cultural adaptation required → CHO + native speaker review required

4. AIGC Compliance Enforcement:
   - Pre-release: Automated AIGC label check (100% coverage required)
   - Post-release: Random audit (5% of all AIGC translations weekly)
   - Violation: Translation blocked, TR-COORD alerted, retraining triggered

5. CISO Security for Translation:
...
6. Review Turnaround SLA:
   - Routine translation: 4h
   - Legal translation: 48h
   - Marketing translation: 12h
   - Emergency translation: 2h
```
